...
-(void) NavgateItem:(id<CMNavgateItemDelegate>)control item:(TBrowserItem&)item
{
    ...
    NSString* sTitle = [NSString stringWithUTF8String:item.sTitle];
    NSString* sUrl   = [NSString stringWithUTF8String:item.sUrl];
    NSString* sID    = [NSString stringWithUTF8String:item.sID];
	...
	if ([sType isEqualToString:@"text/html"] || [sType isEqualToString:@"text/vnd.wap.wml"]) {
        ...
        if ([tool isExistWifi])
        {
            ...
            playerControl->bIsHiddenList = YES;
