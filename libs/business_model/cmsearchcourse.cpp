...
struct node
{
	CMList<TSearchItem*> *tempKey;
	node()
	{
		tempKey =  new CMList<TSearchItem*>;
	}
	~node()
	{
		SAFEDELETE(tempKey);
...
//			CM_ERRP("the id:%d",id);
			head[id].tempKey->push_back(item);
//			CM_ERRP("the head x:%d;y:%d",id,head[id].count);
		...
		for(int i = 0;i < 10;i++)
		{
			if(head[i].tempKey->size()!=0)
			{
				for(int j=0;j<head[i].tempKey->size();j++)
				{
					m_lstItem->push_back(head[i].tempKey->at(j));
					num++;
				}
				//清空
				head[i].tempKey->clear();
			}
		}
		pos++;
