...
- (void)readResponseHeaders
{
	...
	CFRelease(message);
	[self performSelectorOnMainThread:@selector(requestReceivedResponseHeaders:) withObject:[[[self responseHeaders] copy] autorelease] waitUntilDone:YES];//[NSThread isMainThread]
}
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if (dataWillBeHandledExternally) {
			...
			[self performSelectorOnMainThread:@selector(passOnReceivedData:) withObject:data waitUntilDone:YES];
