...
//
// Modified by 廖宏兴 on 2015.4.28 to customizing the pullToRefreshView
// www.wunding.com
// 为了适配云学习的下拉刷新视图的原风格，修改过此库的源码，不适合用pod管理
...
#define kArrowColor   [UIColor colorWithRed:141.0/255.0 green:154.0/255.0 blue:172.0/255.0 alpha:1.0]
#define kBackgroundColor [UIColor colorWithRed:216/255.0 green:219/255.0 blue:220/255.0 alpha:1.0]

static CGFloat const SVPullToRefreshViewTriggeredHeight = 65;
static CGFloat const SVPullToRefreshViewHeight = 600;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        [self setArrowColor:kArrowColor];
        self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        self.backgroundColor = kBackgroundColor;
        self.state = SVPullToRefreshStateStopped;
        self.showsDateLabel = NO;
        
        self.titles = [NSMutableArray arrayWithObjects:I(@"下拉刷新..."),
                            I(@"释放刷新..."),
                             I(@"加载中…"),
                                nil];
        ...
        self.subtitleLabel.textColor = kTextColor;
        self.subtitleLabel.shadowColor = [UIColor colorWithWhite:0.9f alpha:1.0f];
        self.subtitleLabel.shadowOffset = CGSizeMake(0.0f, 1.0f);
        self.subtitleLabel.backgroundColor = [UIColor clearColor];
        self.subtitleLabel.textAlignment = UITextAlignmentCenter;
        
        NSString *lastUpdatedString = [[NSUserDefaults standardUserDefaults] objectForKey:@"PullToRefreshView_LastRefresh"];
        lastUpdatedString = (lastUpdatedString != nil) ? lastUpdatedString : I(@"从未更新");
        
        [self setSubtitle:lastUpdatedString forState:SVPullToRefreshStateAll];
...
- (void)refreshLastUpdatedDate{
    ...
    [formatter setAMSymbol:I(@"上午")];
    [formatter setPMSymbol:I(@"下午")];
    [formatter setDateFormat:@"yyyy/MM/dd a hh:mm"];
    NSString *dateString = [NSString stringWithFormat:@"%@: %@",I(@"上次更新"), [formatter stringFromDate:date]];
    [self setSubtitle:dateString forState:SVPullToRefreshStateAll] ;
    [[NSUserDefaults standardUserDefaults] setObject:dateString forKey:@"PullToRefreshView_LastRefresh"];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        [self.titleLabel setFrame:CGRectMake(0.0f, self.frame.size.height - 48.0f, self.frame.size.width, 20.0f)];
        [self.subtitleLabel setFrame:CGRectMake(0.0f, self.frame.size.height - 30.0f, self.frame.size.width, 20.0f)];
        
        
        self.arrow.frame = CGRectMake(25.0f, self.frame.size.height - 65.0f, 30.0f, 55.0f);
...
- (void)setScrollViewContentInsetForLoading {
    ...
    switch (self.position) {
        case SVPullToRefreshPositionTop:
            currentInsets.top = MIN(offset, self.originalTopInset + SVPullToRefreshViewTriggeredHeight);
            break;
        case SVPullToRefreshPositionBottom:
            currentInsets.bottom = MIN(offset, self.originalBottomInset + SVPullToRefreshViewTriggeredHeight);
            break;
    }
    [self setScrollViewContentInset:currentInsets];
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                scrollOffsetThreshold = -SVPullToRefreshViewTriggeredHeight - self.originalTopInset;
                break;
            case SVPullToRefreshPositionBottom:
                scrollOffsetThreshold = MAX(self.scrollView.contentSize.height - self.scrollView.bounds.size.height, 0.0f) + SVPullToRefreshViewTriggeredHeight + self.originalBottomInset;
                break;
        }
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                ...
                offset = MIN(offset, self.originalTopInset + SVPullToRefreshViewTriggeredHeight);
                contentInset = self.scrollView.contentInset;
                self.scrollView.contentInset = UIEdgeInsetsMake(offset, contentInset.left, contentInset.bottom, contentInset.right);
                break;
            case SVPullToRefreshPositionBottom:
                if (self.scrollView.contentSize.height >= self.scrollView.bounds.size.height) {
                    offset = MAX(self.scrollView.contentSize.height - self.scrollView.bounds.size.height + SVPullToRefreshViewTriggeredHeight, 0.0f);
                    offset = MIN(offset, self.originalBottomInset + SVPullToRefreshViewTriggeredHeight);
                    contentInset = self.scrollView.contentInset;
                    ...
                    offset = MIN(SVPullToRefreshViewTriggeredHeight, self.originalBottomInset + SVPullToRefreshViewTriggeredHeight);
                    contentInset = self.scrollView.contentInset;
                    self.scrollView.contentInset = UIEdgeInsetsMake(-offset, contentInset.left, contentInset.bottom, contentInset.right);
                }
                break;
    ...
    switch (self.position) {
        case SVPullToRefreshPositionTop:
            ...
            if(fequalzero(self.scrollView.contentOffset.y)) {
                [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentOffset.x, -SVPullToRefreshViewTriggeredHeight) animated:YES];
