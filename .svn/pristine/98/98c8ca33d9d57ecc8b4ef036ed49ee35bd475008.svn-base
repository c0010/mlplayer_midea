...
const CGFloat kArrowSize = 12.0f;

@interface qa_ChoiceListView : UIView{
    NSArray                 *_menuItems;
}
//@property (nonatomic, retain) NSArray *menuItems;
@end

@interface qa_ChoiceListOverlay : UIView
@end

@implementation qa_ChoiceListOverlay
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touched == self) {
        ...
        for (UIView *v in self.subviews) {
            if ([v isKindOfClass:[qa_ChoiceListView class]]
...
@implementation qa_ChoiceListItem
...
+ (instancetype) menuItem:(NSString *) title
                   ...
                   action:(SEL) action
{
    return [[qa_ChoiceListItem alloc] init:title
                              image:image
                             target:target
                             action:action];
}
...
@implementation qa_ChoiceListView {
             ...
             menuItems:(NSArray *)menuItems
{
    ...
    qa_ChoiceListOverlay *overlay = [[qa_ChoiceListOverlay alloc] initWithFrame:view.bounds];
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.2
                             animations:^(void) {
                                 ...
                                 if ([self.superview isKindOfClass:[qa_ChoiceListOverlay class]])
...
- (void)performAction:(id)sender
{
    ...
    qa_ChoiceListItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView
{
    ...
    const CGFloat kMinMenuItemHeight = 38.f;
    const CGFloat kMinMenuItemWidth = 32.f;
    const CGFloat kMarginX = 10.f;
    const CGFloat kMarginY = 5.f;
    
    UIFont *titleFont = [qa_ChoiceList titleFont];
    ...
    for (qa_ChoiceListItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [qa_ChoiceListView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [qa_ChoiceListView gradientLine: (CGSize){maxItemWidth, 1}];
    ...
    for (qa_ChoiceListItem *menuItem in _menuItems) {
        ...
        if (itemNum < _menuItems.count - 1) {
            ...
            gradientView.frame = (CGRect){0.0, maxItemHeight + 1, gradientLine.size};
...
+ (UIImage *) selectedImage: (CGSize) size
{
    ...
    const CGFloat components[] = {
        238/255.0, 238/255.0, 238/255.0, 1,
        238/255.0, 238/255.0, 238/255.0, 1,
    };
...
+ (UIImage *) gradientLine: (CGSize) size
{
    ...
    const CGFloat R = 238/255.0, G = 238/255.0, B = 238/255.0;
             ...
             inContext:(CGContextRef) context
{
    ...
    UIColor *tintColor = [qa_ChoiceList tintColor];
    if (tintColor) {
...
static qa_ChoiceList *gMenu;
static UIColor *gTintColor;
static UIFont *gTitleFont;
@implementation qa_ChoiceList{
    qa_ChoiceListView *_menuView;
    BOOL        _observing;
}
+ (instancetype) sharedMenu
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        gMenu = [[qa_ChoiceList alloc] init];
    });
    return gMenu;
}
              ...
              menuItems:(NSArray *)menuItems
{
    ...
    _menuView = [[qa_ChoiceListView alloc] init];
...
{
    [[self sharedMenu] showMenuInView:view fromRect:rect menuItems:menuItems transform:transform];
}
