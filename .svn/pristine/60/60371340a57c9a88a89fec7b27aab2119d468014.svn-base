...
@interface CMTrainSignController ()

@end

@implementation CMTrainSignController
...
- (void)viewDidLoad {
    ...
    [m_pData addObject:I(@"未签到")];
    [m_pData addObject:I(@"已签到")];
    ...
    selview.frame = CGRectMake(0, 5, 3.0, UI_IOS_WINDOW_WIDTH/2.0 - 10);
    selview.backgroundColor = kColorForeground;
    [m_pTableView addSubview:selview];
    
    //数据列表
    listView = [[CMTrainSignView alloc]initWithFrame:CGRectMake(0, 40, UI_IOS_WINDOW_WIDTH, UI_IOS_WINDOW_HEIGHT-60-44)];
    ...
    strcpy(listView->m_sFlag, "nosignin");
...
- (void)swipeResult:(UISwipeGestureRecognizer *)tap
{
    ...
    if (nSelectRow != index)
    {
        ...
        [UIView animateWithDuration:0.3 animations:^(void){
            float width;
            float cellwidth;
            width = UI_IOS_WINDOW_WIDTH/2.0 - 10;
            
            cellwidth = UI_IOS_WINDOW_WIDTH/2.0;
...
- (IBAction)btnPressed:(id)sender
{
    ...
    if (nSelectRow != indexPath.row)
    {
        ...
        [UIView animateWithDuration:0.3 animations:^(void){
            ...
            newframe.size.height = UI_IOS_WINDOW_WIDTH / 2.0 - 10;
            selview.frame = newframe;
...
- (void)tabChange:(NSInteger)tabIndex
{
    if (tabIndex == 0) {
        ...
        strcpy(listView->m_sFlag, "signin");
        listView.flag=1;
    }
